...
void NuevaLinea(){
	...
	if (banInicio==1){
		if (banFile==0){
			pthread_create (&h1, NULL, thGrabarVector, (void*)tramaDatos);   	//Crea un hilo h1 para guardar el vector tramaDatos en el archivo binario
...
void NuevaMuestra(){
    ...
    if (banLinea==1){
		...
		if (numBytes==11){														//Guardo 10 muestras
		   ...
		   tramaDatos[1] = contCiclos;
		   pthread_create (&h1, NULL, thGrabarVector, (void*)tramaDatos);   	//Crea un hilo h1 para guardar el vector tramaDatos en el archivo binario
		   pthread_join (h1, NULL);												//Une el hilo h1 al hilo principal
		}
...
void *thGrabarVector(void *arg) {
	...
	if (fp!=NULL){
		do{
		outFwrite = fwrite(trama, sizeof(char), NUM_ELEMENTOS, fp);	
