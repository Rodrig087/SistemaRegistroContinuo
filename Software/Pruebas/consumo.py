...
def anadirLinea(posicion,texto):
	contenido=file("/media/PenDrive/Consumo/consumo.txt").read().splitlines()
	contenido.insert(posicion,texto)
	f=file("/media/PenDrive/Consumo/consumo.txt", "w")
	f.writelines("\n".join(contenido))
...
if os.path.isfile("/media/PenDrive/Consumo/consumo.txt")==False:
	f=open("/media/PenDrive/Consumo/consumo.txt", "w")
...
try:
	...
	while(1):
		...
		if tiempo.tm_sec==0 and tiempo.tm_min%10==0:
			...
			f=open("/media/PenDrive/Consumo/consumo.txt", "a+")
			f.write(dato)
			f.close()
