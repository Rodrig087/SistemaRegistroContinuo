...
//Funcion para rellenar y enviar la trama de respuesta
//////////////////////////////////////// Formato de trama ////////////////////////////////////////
...
void enviarTrama(unsigned short dataSize){
     ...
     unsigned short rSize = dataSize + 7;                //Longitud de la trama de respuesta
     
      //Prepara la trama PDU
     PDU[0] = Add;                                       //Rellena el campo de direccion con la direccion del dispositivo esclavo
     PDU[1] = Ptcn[2];                                   //Rellena el campo de funcion con la funcion requerida en la trama de peticion
     ...
     PDU[2] = 0xEE;                                     //Rellena el campo de datos con los valores 0xAAFF
     PDU[3] = 0xEE;
     ...
     Rspt[dataSize+3] = *(ptrCRC16+1);                  //Rellena el campo CRC_MSB de la trama de respuesta
     Rspt[dataSize+4] = *ptrCRC16;                      //Rellena el campo CRC_LSB de la trama de respuesta
     Rspt[dataSize+5] = End1;                           //Se rellena el penultimo byte de la trama de repuesta con el primer byte del delimitador de final de trama
     Rspt[dataSize+6] = End2;                           //Se rellena el ultimo byte de la trama de repuesta con el segundo byte del delimitador de final de trama

     //Envia la trama de respuesta por el UART1
...
void Configuracion(){
     ...
     IPR1.RC1IP = 0;                                   //Nivel de prioridad de USART1 = Baja prioridad
     IPR1.TMR1IP = 1;                                  //Nivel de prioridad de TMR1 = Alta prioridad
...
void main() {
     ...
     RC5_bit = 0;                                             //Inicializa el Max485 en modo de lectura;
     RC4_bit = 0;
              ...
              if (Ptcn[1]==Add){                              //Verifica que el campo de direccion de la trama de respuesta concuerde con la direccion del esclavo solicitado
                  ...
                  *(ptrCRCPDU+1) = Ptcn[Psize-4];             //Asigna el elemento CRC_MSB de la trama de respuesta al MSB de la variable CRCPDU
                  ...
                  if (CRC16==CRCPDU) {                        //Verifica si el CRC calculado sea igual al CRC obtenido de la trama de peticion
                     ...
                     enviarTrama(2);                          //Se envia una trama de respuesta con 2 bytes de payload
