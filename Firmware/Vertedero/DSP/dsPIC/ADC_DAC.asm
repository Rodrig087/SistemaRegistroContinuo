...
;ADC_DAC.c,146 :: 		T2CON.TON = 1;                                                            //Enciende el TMR2
	BSET	T2CON, #15
;ADC_DAC.c,147 :: 		i = 0;                                                                    //Limpia las variables asociadas al almacenamiento de la señal muestreada
	CLR	W0
	MOV	W0, _i
...
L__Pulso164:
	...
	GOTO	L_Pulso15
L_Pulso16:
;ADC_DAC.c,174 :: 		YY = (unsigned int)(y0);                                          //Reconstrucción de la señal: y en 10 bits.
	MOV	_y0, W0
	MOV	_y0+2, W1
...
;ADC_DAC.c,177 :: 		}
	GOTO	L_Pulso8
L_Pulso9:
;ADC_DAC.c,179 :: 		bm = 2;                                                               //Cambia el estado de la bandera bm para dar paso al cálculo del pmax y TOF
	MOV	#lo_addr(_bm), W1
	MOV.B	#2, W0
	MOV.B	W0, [W1]
;ADC_DAC.c,181 :: 		}
...
;ADC_DAC.c,448 :: 		bm = 1;                                    //Cambia el valor de la bandera bm para terminar con el muestreo y dar comienzo al procesamiento de la señal
	MOV	#lo_addr(_bm), W1
	MOV.B	#1, W0
	MOV.B	W0, [W1]
...
;ADC_DAC.c,494 :: 		AD1CON1.SIMSAM = 0;                                                        //0 -> Muestrea múltiples canales individualmente en secuencia
	BCLR	AD1CON1, #3
...
;ADC_DAC.c,500 :: 		AD1CON2.CSCNA = 0;                                                         //No escanea las entradas de CH0 durante la Muestra A
	BCLR	AD1CON2, #10
;ADC_DAC.c,501 :: 		AD1CON2.BUFM = 0;                                                          //Bit de selección del modo de relleno del búfer, 0 -> Siempre comienza a llenar el buffer desde el principio
	BCLR	AD1CON2, #1
;ADC_DAC.c,502 :: 		AD1CON2.ALTS = 0x00;                                                       //Utiliza siempre la selección de entrada de canal para la muestra A
	BCLR	AD1CON2, #0
