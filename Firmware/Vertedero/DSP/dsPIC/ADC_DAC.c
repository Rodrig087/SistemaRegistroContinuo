...
#include <CALCULO_VELOCIDADSONIDO.c>
...
int Distancia(){
     ...
     while (conts<Nsm){
           Pulso();                           //Inicia una secuencia de medicion
           T2b = T2;
           if ((T2b-T2a)<=T2umb){             //Verifica si el T2 actual esta dentro de un umbral pre-establecido
              T2sum = T2sum + T2b;            //Acumula la sumatoria de valores de T2 calculados por la funcion Pulso()
     ...
     vSonido = CalcularVelocidadSonido();     //Calcula la velocidad del sonido
     //VSnd = 343.2;

     TOF = (T1+T2prom-T2adj)/1.0e6;           //Calcula el TOF en seg
     Dst = (vSonido*TOF/2.0) * 1000.0;           //Calcula la distancia en mm
     doub = modf(Dst, &iptr);
     if (doub>=0.5){
        Dst=ceil(Dst);
     }else{
...
void Responder(unsigned int Reg){
     ...
     vSonido = CalcularVelocidadSonido();                             //**Revisar: El programa necesita ejecutar esta funcion para armar la trama de datos de forma correcta, deconosco el porque
     ...
     chTemp = (unsigned char *) & TemperaturaInt;//Asocia el valor calculado de Temperatura al puntero chTemp
...
void main() {
     ...
     Calcular();                                                                //Realiza una secuencia de calculo inicial para tener algo que responder cunado se le pida
