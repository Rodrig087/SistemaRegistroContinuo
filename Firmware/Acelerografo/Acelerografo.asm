...
L_ADXL355_init2:
	CP.B	W10, #1
	BRA NZ	L__ADXL355_init141
	GOTO	L_ADXL355_init4
L__ADXL355_init141:
	CP.B	W10, #2
	BRA NZ	L__ADXL355_init142
	GOTO	L_ADXL355_init5
L__ADXL355_init142:
	CP.B	W10, #4
	BRA NZ	L__ADXL355_init143
	GOTO	L_ADXL355_init6
L__ADXL355_init143:
	CP.B	W10, #8
	BRA NZ	L__ADXL355_init144
...
;adxl355_spi.c,151 :: 		if((ADXL355_read_byte(Status)&0x01)==1){                                 //Verifica que el bit DATA_RDY del registro Status este en alto
	...
	AND	W0, #1, W0
	CP	W0, #1
	BRA Z	L__ADXL355_read_data148
	GOTO	L_ADXL355_read_data8
L__ADXL355_read_data148:
...
; j start address is: 4 (W2)
	CP.B	W2, #9
	BRA LTU	L__ADXL355_read_data149
	GOTO	L_ADXL355_read_data10
L__ADXL355_read_data149:
...
; j start address is: 4 (W2)
	CP.B	W2, #9
	BRA LTU	L__ADXL355_read_data150
	GOTO	L_ADXL355_read_data14
L__ADXL355_read_data150:
...
;Acelerografo.c,74 :: 		ConfigurarGPS();
	CALL	_ConfigurarGPS
;Acelerografo.c,75 :: 		Delay_ms(1000);
	MOV	#123, W8
	MOV	#4681, W7
L_main22:
	DEC	W7
...
L__Muestrear173:
	...
	MOV	W1, W0
	CP	W0, #0
	BRA NZ	L__Muestrear174
	GOTO	L__Muestrear107
L__Muestrear174:
	GOTO	L_Muestrear42
L__Muestrear108:
L__Muestrear107:
...
L_Muestrear30:
L_Muestrear29:
;Acelerografo.c,257 :: 		contCiclos++;                                                              //Incrementa el contador de ciclos
	MOV.B	#1, W1
	MOV	#lo_addr(_contCiclos), W0
	ADD.B	W1, [W0], [W0]
;Acelerografo.c,259 :: 		}
...
;Acelerografo.c,264 :: 		void InterrupcionP2(){
;Acelerografo.c,266 :: 		if (INT1IE_bit==0){
	BTSC	INT1IE_bit, BitPos(INT1IE_bit+0)
	GOTO	L_InterrupcionP249
;Acelerografo.c,267 :: 		INT1IE_bit = 1;
	BSET	INT1IE_bit, BitPos(INT1IE_bit+0)
;Acelerografo.c,268 :: 		}
L_InterrupcionP249:
;Acelerografo.c,270 :: 		RP2 = 1;
	BSET	LATB4_bit, BitPos(LATB4_bit+0)
;Acelerografo.c,271 :: 		Delay_us(20);
	MOV	#160, W7
L_InterrupcionP250:
	DEC	W7
	BRA NZ	L_InterrupcionP250
	NOP
	NOP
;Acelerografo.c,272 :: 		RP2 = 0;
	BCLR	LATB4_bit, BitPos(LATB4_bit+0)
;Acelerografo.c,273 :: 		}
...
;Acelerografo.c,283 :: 		void spi_1() org  IVT_ADDR_SPI1INTERRUPT {
;Acelerografo.c,285 :: 		SPI1IF_bit = 0;                                                            //Limpia la bandera de interrupcion por SPI
	PUSH	W10
	PUSH	W11
	PUSH	W12
	PUSH	W13
	BCLR	SPI1IF_bit, BitPos(SPI1IF_bit+0)
;Acelerografo.c,286 :: 		buffer = SPI1BUF;                                                          //Guarda el contenido del bufeer (lectura)
	MOV	#lo_addr(_buffer), W1
	MOV.B	SPI1BUF, WREG
	MOV.B	W0, [W1]
;Acelerografo.c,289 :: 		if (banMuestrear==0){
	...
	GOTO	L_spi_152
L__spi_1178:
;Acelerografo.c,290 :: 		if (buffer==0xA0){
	...
	GOTO	L_spi_153
L__spi_1179:
;Acelerografo.c,291 :: 		banMuestrear = 1;                                                    //Cambia el estado de la bandera para que no inicie el muestreo mas de una vez de manera consecutiva
	MOV	#lo_addr(_banMuestrear), W1
	MOV.B	#1, W0
	MOV.B	W0, [W1]
...
;Acelerografo.c,305 :: 		}
L_spi_152:
;Acelerografo.c,308 :: 		if (banMuestrear==1){
	MOV	#lo_addr(_banMuestrear), W0
	MOV.B	[W0], W0
	CP.B	W0, #1
	BRA Z	L__spi_1180
	GOTO	L_spi_155
L__spi_1180:
;Acelerografo.c,309 :: 		if (buffer==0xAF){
	MOV	#lo_addr(_buffer), W0
	MOV.B	[W0], W1
	MOV.B	#175, W0
	CP.B	W1, W0
	...
	GOTO	L_spi_156
L__spi_1181:
;Acelerografo.c,310 :: 		banInicio = 0;                                                       //Bandera que permite el inicio del muestreo dentro de la interrupcion INT1
	MOV	#lo_addr(_banInicio), W1
	CLR	W0
	MOV.B	W0, [W1]
;Acelerografo.c,311 :: 		banMuestrear = 0;                                                    //Cambia el estado de la bandera para permitir que inicie el muestreo de nuevo en el futuro
	MOV	#lo_addr(_banMuestrear), W1
	CLR	W0
	MOV.B	W0, [W1]
;Acelerografo.c,312 :: 		ADXL355_write_byte(POWER_CTL, DRDY_OFF|STANDBY);                     //Coloco el ADXL en modo STANDBY para pausar las conversiones y limpiar el FIFO
	MOV.B	#5, W11
	MOV.B	#45, W10
	CALL	_ADXL355_write_byte
;Acelerografo.c,314 :: 		if (INT1IE_bit==1){
	BTSS	INT1IE_bit, BitPos(INT1IE_bit+0)
	GOTO	L_spi_157
;Acelerografo.c,315 :: 		INT1IE_bit = 0;
	BCLR	INT1IE_bit, BitPos(INT1IE_bit+0)
;Acelerografo.c,316 :: 		}
L_spi_157:
;Acelerografo.c,318 :: 		if (T1CON.TON==1){
	BTSS	T1CON, #15
	GOTO	L_spi_158
;Acelerografo.c,319 :: 		T1CON.TON = 0;
	BCLR	T1CON, #15
;Acelerografo.c,320 :: 		}
L_spi_158:
;Acelerografo.c,321 :: 		}
...
;Acelerografo.c,322 :: 		}
L_spi_155:
;Acelerografo.c,326 :: 		if ((banSetReloj==0)){
	...
	GOTO	L_spi_159
L__spi_1182:
;Acelerografo.c,327 :: 		if (buffer==0xC0){
	MOV	#lo_addr(_buffer), W0
	MOV.B	[W0], W1
	MOV.B	#192, W0
	CP.B	W1, W0
	...
	GOTO	L_spi_160
L__spi_1183:
;Acelerografo.c,328 :: 		banTIGPS = 0;                                                        //Limpia la bandera de inicio de trama  del GPS
	MOV	#lo_addr(_banTIGPS), W1
	CLR	W0
	MOV.B	W0, [W1]
;Acelerografo.c,329 :: 		banTCGPS = 0;                                                        //Limpia la bandera de trama completa
	MOV	#lo_addr(_banTCGPS), W1
	CLR	W0
	MOV.B	W0, [W1]
;Acelerografo.c,330 :: 		i_gps = 0;                                                           //Limpia el subindice de la trama GPS
	CLR	W0
	MOV	W0, _i_gps
;Acelerografo.c,332 :: 		if (U1RXIE_bit==0){
	BTSC	U1RXIE_bit, BitPos(U1RXIE_bit+0)
	GOTO	L_spi_161
;Acelerografo.c,333 :: 		U1RXIE_bit = 1;
...
;Acelerografo.c,336 :: 		}
L_spi_159:
;Acelerografo.c,339 :: 		if ((banSetReloj==0)&&(buffer==0xA8)){
	MOV	#lo_addr(_banSetReloj), W0
	MOV.B	[W0], W0
	CP.B	W0, #0
	BRA Z	L__spi_1184
...
L__spi_1185:
L__spi_1117:
;Acelerografo.c,340 :: 		horaSistema = RecuperarHoraRTC();                                       //Recupera la hora del RTC
	CALL	_RecuperarHoraRTC
	MOV	W0, _horaSistema
	MOV	W1, _horaSistema+2
;Acelerografo.c,341 :: 		fechaSistema = RecuperarFechaRTC();                                     //Recupera la fecha del RTC
	CALL	_RecuperarFechaRTC
	MOV	W0, _fechaSistema
	MOV	W1, _fechaSistema+2
;Acelerografo.c,342 :: 		AjustarTiempoSistema(horaSistema, fechaSistema, tiempo);                //Actualiza los datos de la trama tiempo con la hora y fecha recuperadas
	MOV.D	W0, W12
	MOV	_horaSistema, W10
	MOV	_horaSistema+2, W11
	MOV	#lo_addr(_tiempo), W0
	...
	CALL	_AjustarTiempoSistema
	SUB	#2, W15
;Acelerografo.c,343 :: 		banEsc = 0;
	MOV	#lo_addr(_banEsc), W1
	CLR	W0
	MOV.B	W0, [W1]
;Acelerografo.c,344 :: 		banSetReloj = 1;
	MOV	#lo_addr(_banSetReloj), W1
	MOV.B	#1, W0
	MOV.B	W0, [W1]
;Acelerografo.c,345 :: 		InterrupcionP2();
	CALL	_InterrupcionP2
;Acelerografo.c,339 :: 		if ((banSetReloj==0)&&(buffer==0xA8)){
L__spi_1119:
L__spi_1118:
;Acelerografo.c,349 :: 		if ((banSetReloj==0)&&(buffer==0xC3)){                                     //**Segun esto solo puedo igualar el reloj una sola vez al encender o reiniciar el dsPIC
	MOV	#lo_addr(_banSetReloj), W0
	MOV.B	[W0], W0
	CP.B	W0, #0
	BRA Z	L__spi_1186
...
L__spi_1187:
L__spi_1116:
;Acelerografo.c,350 :: 		banEsc = 1;
	MOV	#lo_addr(_banEsc), W1
	MOV.B	#1, W0
	MOV.B	W0, [W1]
;Acelerografo.c,351 :: 		j = 0;
	CLR	W0
	MOV	W0, _j
;Acelerografo.c,349 :: 		if ((banSetReloj==0)&&(buffer==0xC3)){                                     //**Segun esto solo puedo igualar el reloj una sola vez al encender o reiniciar el dsPIC
L__spi_1121:
L__spi_1120:
;Acelerografo.c,353 :: 		if ((banEsc==1)&&(buffer!=0xC3)&&(buffer!=0xC4)){
...
L__spi_1190:
L__spi_1115:
;Acelerografo.c,354 :: 		tiempoRPI[j] = buffer;
	MOV	#lo_addr(_tiempoRPI), W1
	MOV	#lo_addr(_j), W0
	ADD	W1, [W0], W1
	MOV	#lo_addr(_buffer), W0
	MOV.B	[W0], [W1]
;Acelerografo.c,355 :: 		j++;
	MOV	#1, W1
	MOV	#lo_addr(_j), W0
	ADD	W1, [W0], [W0]
;Acelerografo.c,353 :: 		if ((banEsc==1)&&(buffer!=0xC3)&&(buffer!=0xC4)){
...
L__spi_1123:
L__spi_1122:
;Acelerografo.c,357 :: 		if ((banEsc==1)&&(buffer==0xC4)){
	MOV	#lo_addr(_banEsc), W0
	MOV.B	[W0], W0
	CP.B	W0, #1
	BRA Z	L__spi_1191
...
L__spi_1192:
L__spi_1114:
;Acelerografo.c,360 :: 		DS3234_init();                                                          //inicializa el RTC
	CALL	_DS3234_init
;Acelerografo.c,362 :: 		horaSistema = RecuperarHoraRTC();                                       //Recupera la hora del RTC
	CALL	_RecuperarHoraRTC
	MOV	W0, _horaSistema
	MOV	W1, _horaSistema+2
;Acelerografo.c,363 :: 		fechaSistema = RecuperarFechaRTC();                                     //Recupera la fecha del RTC
	CALL	_RecuperarFechaRTC
	MOV	W0, _fechaSistema
	MOV	W1, _fechaSistema+2
;Acelerografo.c,364 :: 		AjustarTiempoSistema(horaSistema, fechaSistema, tiempo);                //Actualiza los datos de la trama tiempo con la hora y fecha recuperadas
	MOV.D	W0, W12
	MOV	_horaSistema, W10
	MOV	_horaSistema+2, W11
	MOV	#lo_addr(_tiempo), W0
	...
	CALL	_AjustarTiempoSistema
	SUB	#2, W15
;Acelerografo.c,365 :: 		banEsc = 0;
	MOV	#lo_addr(_banEsc), W1
	CLR	W0
	MOV.B	W0, [W1]
;Acelerografo.c,366 :: 		banSetReloj = 1;
	MOV	#lo_addr(_banSetReloj), W1
	MOV.B	#1, W0
	MOV.B	W0, [W1]
;Acelerografo.c,367 :: 		InterrupcionP2();
	CALL	_InterrupcionP2
;Acelerografo.c,357 :: 		if ((banEsc==1)&&(buffer==0xC4)){
L__spi_1126:
L__spi_1125:
;Acelerografo.c,371 :: 		if (banSetReloj==1){
	MOV	#lo_addr(_banSetReloj), W0
	MOV.B	[W0], W0
	CP.B	W0, #1
	BRA Z	L__spi_1193
	GOTO	L_spi_174
L__spi_1193:
;Acelerografo.c,372 :: 		banSetReloj = 2;
	MOV	#lo_addr(_banSetReloj), W1
	MOV.B	#2, W0
	MOV.B	W0, [W1]
;Acelerografo.c,373 :: 		j = 0;
	CLR	W0
	MOV	W0, _j
;Acelerografo.c,374 :: 		SPI1BUF = tiempo[j];
	MOV	#lo_addr(_tiempo), W1
	MOV	#lo_addr(_j), W0
	ADD	W1, [W0], W0
	MOV.B	[W0], W0
...
;Acelerografo.c,375 :: 		}
L_spi_174:
;Acelerografo.c,376 :: 		if ((banSetReloj==2)&&(buffer!=0xC1)){
	MOV	#lo_addr(_banSetReloj), W0
	MOV.B	[W0], W0
	CP.B	W0, #2
	BRA Z	L__spi_1194
...
L__spi_1195:
L__spi_1113:
;Acelerografo.c,377 :: 		SPI1BUF = tiempo[j];
	MOV	#lo_addr(_tiempo), W1
	MOV	#lo_addr(_j), W0
	ADD	W1, [W0], W0
	MOV.B	[W0], W0
...
L__spi_1128:
L__spi_1127:
;Acelerografo.c,380 :: 		if ((banSetReloj==2)&&(buffer==0xC1)){                                     //Si detecta el delimitador de final de trama:
	MOV	#lo_addr(_banSetReloj), W0
	MOV.B	[W0], W0
	CP.B	W0, #2
	BRA Z	L__spi_1196
...
L__spi_1197:
L__spi_1112:
;Acelerografo.c,381 :: 		banSetReloj = 0;                                                        //Limpia la bandera de lectura
	MOV	#lo_addr(_banSetReloj), W1
	CLR	W0
	MOV.B	W0, [W1]
...
L__spi_1130:
L__spi_1129:
;Acelerografo.c,387 :: 		if ((banLec==1)&&(buffer==0xB0)){                                          //Verifica si la bandera de inicio de trama esta activa
...
L__spi_1199:
L__spi_1111:
;Acelerografo.c,388 :: 		banLec = 2;                                                             //Activa la bandera de lectura
	MOV	#lo_addr(_banLec), W1
	MOV.B	#2, W0
	MOV.B	W0, [W1]
;Acelerografo.c,389 :: 		i = 0;
	CLR	W0
	MOV	W0, _i
...
L__spi_1132:
L__spi_1131:
;Acelerografo.c,392 :: 		if ((banLec==2)&&(buffer!=0xB1)){
	MOV	#lo_addr(_banLec), W0
	MOV.B	[W0], W0
	CP.B	W0, #2
	BRA Z	L__spi_1200
...
L__spi_1201:
L__spi_1110:
;Acelerografo.c,393 :: 		SPI1BUF = tramaCompleta[i];
	MOV	#lo_addr(_tramaCompleta), W1
	MOV	#lo_addr(_i), W0
	ADD	W1, [W0], W0
	MOV.B	[W0], W0
...
L__spi_1134:
L__spi_1133:
;Acelerografo.c,396 :: 		if ((banLec==2)&&(buffer==0xB1)){                                          //Si detecta el delimitador de final de trama:
	MOV	#lo_addr(_banLec), W0
	MOV.B	[W0], W0
	CP.B	W0, #2
	BRA Z	L__spi_1202
...
L__spi_1203:
L__spi_1109:
;Acelerografo.c,397 :: 		banLec = 0;                                                             //Limpia la bandera de lectura
	MOV	#lo_addr(_banLec), W1
	CLR	W0
	MOV.B	W0, [W1]
...
;Acelerografo.c,406 :: 		void int_1() org IVT_ADDR_INT1INTERRUPT {
;Acelerografo.c,408 :: 		INT1IF_bit = 0;                                                            //Limpia la bandera de interrupcion externa INT1
	BCLR	INT1IF_bit, BitPos(INT1IF_bit+0)
;Acelerografo.c,410 :: 		TEST = ~TEST;
	BTG	LATB12_bit, BitPos(LATB12_bit+0)
;Acelerografo.c,411 :: 		horaSistema++;                                                             //Incrementa el reloj del sistema
	MOV	#1, W1
	MOV	#0, W2
	MOV	#lo_addr(_horaSistema), W0
	ADD	W1, [W0], [W0++]
	ADDC	W2, [W0], [W0--]
;Acelerografo.c,413 :: 		if (horaSistema==86400){                                                   //(24*3600)+(0*60)+(0) = 86400
	MOV	_horaSistema, W2
	...
	GOTO	L_int_190
L__int_1205:
;Acelerografo.c,414 :: 		horaSistema = 0;                                                        //Reinicia el reloj al llegar a las 24:00:00 horas
	CLR	W0
	CLR	W1
	MOV	W0, _horaSistema
	MOV	W1, _horaSistema+2
;Acelerografo.c,415 :: 		}
L_int_190:
;Acelerografo.c,417 :: 		if (banInicio==1){
...
;Acelerografo.c,425 :: 		void Timer1Int() org IVT_ADDR_T1INTERRUPT{
;Acelerografo.c,427 :: 		T1IF_bit = 0;                                                              //Limpia la bandera de interrupcion por desbordamiento del Timer1
	PUSH	W10
	BCLR	T1IF_bit, BitPos(T1IF_bit+0)
;Acelerografo.c,429 :: 		numFIFO = ADXL355_read_byte(FIFO_ENTRIES); //75                            //Lee el numero de muestras disponibles en el FIFO
	MOV.B	#5, W10
	CALL	_ADXL355_read_byte
	MOV	#lo_addr(_numFIFO), W1
	MOV.B	W0, [W1]
;Acelerografo.c,430 :: 		numSetsFIFO = (numFIFO)/3;                 //25                            //Lee el numero de sets disponibles en el FIFO
	ZE	W0, W0
	MOV	#3, W2
	REPEAT	#17
	DIV.S	W0, W2
	...
	MOV	#lo_addr(_numSetsFIFO), W0
	MOV.B	W1, [W0]
;Acelerografo.c,433 :: 		for (x=0;x<numSetsFIFO;x++){
	CLR	W0
	MOV	W0, _x
L_Timer1Int92:
	MOV	#lo_addr(_numSetsFIFO), W0
	...
	GOTO	L_Timer1Int93
L__Timer1Int208:
;Acelerografo.c,434 :: 		ADXL355_read_FIFO(datosLeidos);                                        //Lee una sola posicion del FIFO
	MOV	#lo_addr(_datosLeidos), W10
	CALL	_ADXL355_read_FIFO
;Acelerografo.c,435 :: 		for (y=0;y<9;y++){
	CLR	W0
	MOV	W0, _y
L_Timer1Int95:
	MOV	_y, W0
	CP	W0, #9
	BRA LTU	L__Timer1Int209
	GOTO	L_Timer1Int96
L__Timer1Int209:
;Acelerografo.c,436 :: 		datosFIFO[y+(x*9)] = datosLeidos[y];                               //LLena la trama datosFIFO
	MOV	_x, W1
	MOV	#9, W0
	MUL.UU	W1, W0, W2
	MOV	#lo_addr(_y), W0
...
;Acelerografo.c,438 :: 		}
	GOTO	L_Timer1Int92
L_Timer1Int93:
;Acelerografo.c,441 :: 		for (x=0;x<(numSetsFIFO*9);x++){      //0-224
	CLR	W0
	MOV	W0, _x
L_Timer1Int98:
	MOV	#lo_addr(_numSetsFIFO), W0
	...
	MOV	#lo_addr(_x), W0
	CP	W2, [W0]
	BRA GTU	L__Timer1Int210
	GOTO	L_Timer1Int99
L__Timer1Int210:
;Acelerografo.c,442 :: 		if ((x==0)||(x%9==0)){
	MOV	_x, W0
	CP	W0, #0
	BRA NZ	L__Timer1Int211
...
L__Timer1Int211:
	...
	MOV	W1, W0
	CP	W0, #0
	BRA NZ	L__Timer1Int212
	GOTO	L__Timer1Int138
L__Timer1Int212:
	GOTO	L_Timer1Int103
L__Timer1Int139:
L__Timer1Int138:
;Acelerografo.c,443 :: 		tramaCompleta[contFIFO+contMuestras+x] = contMuestras;
	MOV	#lo_addr(_contMuestras), W0
	ZE	[W0], W1
	MOV	#lo_addr(_contFIFO), W0
	ADD	W1, [W0], W1
	...
	MOV	#lo_addr(_contMuestras), W0
	MOV.B	[W0], [W1]
;Acelerografo.c,444 :: 		tramaCompleta[contFIFO+contMuestras+x+1] = datosFIFO[x];
	MOV	#lo_addr(_contMuestras), W0
	ZE	[W0], W1
	MOV	#lo_addr(_contFIFO), W0
	ADD	W1, [W0], W1
	...
	ADD	W1, [W0], W0
	MOV.B	[W0], [W2]
;Acelerografo.c,445 :: 		contMuestras++;
	MOV.B	#1, W1
	MOV	#lo_addr(_contMuestras), W0
	ADD.B	W1, [W0], [W0]
;Acelerografo.c,446 :: 		} else {
	GOTO	L_Timer1Int104
L_Timer1Int103:
;Acelerografo.c,447 :: 		tramaCompleta[contFIFO+contMuestras+x] = datosFIFO[x];
	MOV	#lo_addr(_contMuestras), W0
	ZE	[W0], W1
	MOV	#lo_addr(_contFIFO), W0
	ADD	W1, [W0], W1
...
;Acelerografo.c,449 :: 		}
	GOTO	L_Timer1Int98
L_Timer1Int99:
;Acelerografo.c,451 :: 		contFIFO = (contMuestras*9);                                               //Incrementa el contador de FIFOs
	MOV	#lo_addr(_contMuestras), W0
	ZE	[W0], W1
	MOV	#9, W0
	MUL.SS	W1, W0, W0
	MOV	W0, _contFIFO
;Acelerografo.c,453 :: 		contTimer1++;                                                              //Incrementa una unidad cada vez que entra a la interrupcion por Timer1
	MOV.B	#1, W1
	MOV	#lo_addr(_contTimer1), W0
	ADD.B	W1, [W0], [W0]
;Acelerografo.c,455 :: 		if (contTimer1==numTMR1){                                                  //Verifica si se cumplio el numero de interrupciones por TMR1 para la tasa de muestreo seleccionada
	MOV	#lo_addr(_contTimer1), W0
	ZE	[W0], W1
	MOV	#lo_addr(_numTMR1), W0
	SE	[W0], W0
	CP	W1, W0
	BRA Z	L__Timer1Int213
	GOTO	L_Timer1Int105
L__Timer1Int213:
;Acelerografo.c,456 :: 		T1CON.TON = 0;                                                          //Apaga el Timer1
	BCLR	T1CON, #15
;Acelerografo.c,457 :: 		banCiclo = 1;                                                           //Activa la bandera que indica que se completo un ciclo de medicion
	MOV	#lo_addr(_banCiclo), W1
	MOV.B	#1, W0
	MOV.B	W0, [W1]
;Acelerografo.c,458 :: 		contTimer1 = 0;                                                         //Limpia el contador de interrupciones por Timer1
	MOV	#lo_addr(_contTimer1), W1
	CLR	W0
	MOV.B	W0, [W1]
;Acelerografo.c,459 :: 		}
